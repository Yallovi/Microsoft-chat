...
export const GetHistoryChatMessages = (
  chats: ChatMessageType[]
): ChatMessage[] => {
  const a = chats?.map((chat) => {
    return {
      messageType: chat?.messageType,
      senderId: chat?.from?.application?.id,
      senderDisplayName: chat?.from?.application?.displayName,
      messageId: chat?.id,
      content: chat?.body?.content,
      createdOn: chat?.createdDateTime,
      mine: true,
      attached: false,
      status: "seen" as MessageStatus,
      contentType: chat?.body?.contentType,
    };
  });

  return a as unknown as ChatMessage[];
  // return [
  //   {
  //     messageType: "chat",
  //     senderId: "user1",

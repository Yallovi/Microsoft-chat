...
export const graphParticipantToACSParticipant = (
  graphParticipant: GraphChatParticipant
): ACSChatParticipant => {
  if (!graphParticipant.userId) {
    throw new Error(
      "Cannot convert graph participant to ACS participant. No ID found on graph participant"
    );
  }
...
export const graphChatMessageToACSChatMessage = (
  ...
  if (!graphMessage.id) {
    throw new Error(
      "Cannot convert graph message to ACS message. No ID found on graph message"
    );
  }
